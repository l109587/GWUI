...
import { Button, Input, message, Modal, TreeSelect, Switch, Tooltip, Space, Tag, Popconfirm } from 'antd';
import { post } from '@/services/https';
import { ExclamationCircleOutlined, EditFilled, ExportOutlined, DeleteFilled, SaveFilled, CloseCircleFilled, CheckCircleFilled, EyeFilled } from '@ant-design/icons';
...
export default (props) => {
	...
	const columnslist = [
		...
		{
      title: language('dmc.cfgmngt.reglist.from'),
      dataIndex: 'from',
      key: 'from',
      align: 'center',
      ellipsis: true,
      width: 100,
      filters: [
        {text: language('dmc.cfgmngt.reglist.report'), value: 'report'},
        {text: language('dmc.cfgmngt.reglist.reg'), value: 'reg'},
      ],
      filterMultiple: false,
      render: (text, record, index) => {
        let color = 'cyan'
        let title = language('dmc.cfgmngt.reglist.report')
        if (record.from == "report") {
          color = 'cyan'
          title = language('dmc.cfgmngt.reglist.report')
        } else {
          color = 'volcano'
          title = language('dmc.cfgmngt.reglist.reg')
        }
        return (
					<AmTag color={color} style={{ padding: '0 10px' }} name={title} />
        )
      }
    },
		{
      ...
      ellipsis: true,
		...
		},
		{
			...
			render: (text, record, index) => {
				let color = 'orange';
				if(record.cert_warn == 1){
					color = 'red';
				}
				if(record.certsn){
					return (
						<AmTag color={color} name={record.certsn} tootipName={ record.cert_warn == 1 ? '该设备注册证书即将过期' : record.certsn} />
					)
				}
			}
		},
