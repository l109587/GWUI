...
import { Button, Input, message, Modal, TreeSelect, Tooltip, Tag, Divider } from 'antd';
import { post } from '@/services/https';
import { ExclamationCircleOutlined, EyeFilled, CloseCircleFilled, CheckCircleFilled } from '@ant-design/icons';
import ProForm, { DrawerForm, ModalForm, ProFormText, ProFormItem } from '@ant-design/pro-form';
import { EditableProTable, ProCard, ProDescriptions } from '@ant-design/pro-components';
...
const { Search } = Input;
let H = document.body.clientHeight - 315;
var clientHeight = H
export default (props) => {
  ...
  const columnslist = [
    ...
    {
      title: language('cfgmngt.devlist.from'),
      dataIndex: 'from',
      key: 'from',
      align: 'center',
      ellipsis: true,
      width: 100,
      filters: [
        { text: language('cfgmngt.devlist.report'), value: 'report' },
        { text: language('cfgmngt.devlist.reg'), value: 'reg' },
      ],
      filterMultiple: false,
      render: (text, record, index) => {
        let color = 'cyan'
        let title = language('cfgmngt.devlist.report')
        if (record.from == "report") {
          color = 'cyan'
          title = language('cfgmngt.devlist.report')
        } else {
          color = 'volcano'
          title = language('cfgmngt.devlist.reg')
        }
        return (
    ...
    {
      title: language('cfgmngt.devlist.uppersystem'),
      dataIndex: 'super_devid',
      align: 'left',
      width: 140,
      ellipsis: true,
    },
		{
			...
			render: (text, record, index) => {
				let name = '';
				let list = devTypeData.length >= 1 ? devTypeData : devTypeList;
				list.map((item) => {
				...
				let color = 'blue';
				if(record.policy_warn == 1){
					color = 'red';
				}
				if(name){
					return (
						<AmTag color={color} style={{maxWidth: '130px', overflow: 'hidden'}} name={name} tootipName={record.policy_warn == 1 ? "该设备暂未下发任何策略" : ''} />
					)
				}
			}
		},
    ...
    {
      title: language('cfgmngt.devlist.publicipaddress'),
    ...
    },
    {
      ...
      render: (text, record, _, action) => [
        record.state == 0 && record.from != "report" ? <>
          <a key="cancel"
            target="_blank"
            onClick={() => {
          ...
          <a key="see"
            ...
            <Tooltip title={language('project.see')} >
              <EyeFilled style={{ width: '16px', fontSize: '16px' }} size={16} />
            </Tooltip>
          </a></> :
          <a key="see"
            onClick={() => {
  ...
  const [columns, setColumns] = useState(columnslist);//draw 弹框状态
  ...
  const getDrawModal = (status, record) => {
    if (status == 1) {
      setDrawStatus(true);
      if(record.zoneName){
        setZoneIdVal(record.zoneID)
        setZoneNameVal(record.zoneName)
  ...
  const dapiColumns = [
    {
      ...
      ellipsis: true,
      render: (text, record) => {
        if (!record.manage || record.manage == 'false') {
      ...
      <DrawerForm
        ...
        <div className='aapplicationinformation'>
          <ProDescriptions column={2}>
            ...
            <ProDescriptions.Item label={language('cfgmngt.devlist.administrativecode')}>{initialValue.address_code ? initialValue.address_code : ' '}</ProDescriptions.Item>
          </ProDescriptions>
          <ProDescriptions column={2}>
            ...
            <ProDescriptions.Item label={language('cfgmngt.devlist.publicipaddress')}>{initialValue.public_ip_address ? initialValue.public_ip_address : ''} </ProDescriptions.Item>
          </ProDescriptions>
        ...
        {drawType == 'see' ?
          <>
            ...
            <ProFormItem label={language('dmc.cfgmngt.reglist.disposalresults')} style={{ marginLeft: '-278px' }}>
              <AmTag name={
            ...
            <div className='groupfrombox'>
              <ProForm.Group style={{ width: "100%", display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
                ...
                <div style={{ marginLeft: '40px' }}>
                  <ProFormText name="zone_id" disabled={zoneStatus} label={language('cfgmngt.devlist.fromzone')} >
        ...
        <div>
          ...
          {drawType == 'see' ?
            <ProCard className='examineresult' title={language('cfgmngt.devlist.examineresult')} ghost>
              <ProFormItem label={language('dmc.cfgmngt.reglist.disposalresults')} style={{ marginLeft: '-107px' }}>
                <AmTag name={
                  initialValue.state == 2 ? language('project.rejected') : initialValue.state == 0 ? language('project.stayexamine') : language('project.passed')}
                  color={initialValue.state == 2 ? 'purple' : initialValue.state == 0 ? 'error' : 'success'} />
                <span style={{ marginLeft: '6px' }}>{initialValue.reason}</span>
