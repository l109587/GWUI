...
import styles from "./drawerPolicy.less";

const { Search } = Input;

const DrawerPolicy = (props) => {
  const {
    visible,
    setVisible,
    operate = "distribute",
    ...
    type,
    module,
    assocurl = "/cfg.php?controller=confPolicy&action=showDevice"
  } = props;
  ...
  const [devID, setDevID] = useState(""); //设备ID
  ...
  const addList = () => {
    ...
    const params = {
      op: operate,
      rule_id: rowInfo?.rule_id,
      zoneID: modalValue == 4 ? "" : zoneId,
      devids: modalValue == 1 ? "" : devidlist.join(","),
    };
  ...
  return (
    <DrawerForm
      ...
      width="800px"
      formRef={drawerRef}
      onFinish={operate === "assoc" ? onClose : addList}
      drawerProps={{
        destroyOnClose: true,
        onClose: onClose,
    ...
    >
      <Row gutter={[8, 8]} className={styles.filterForm}>
        <Col>
          <Search
            placeholder="请输入设备ID"
            onSearch={(value) => {
