...
import { ExclamationCircleOutlined, EditFilled, ExportOutlined, EyeFilled } from '@ant-design/icons';
...
export default (props) => {
    ...
    let filtersType = { onstate: null, verifystate: null, regstate: null };
    const onstateType = props.location?.state?.type;
    console.log(onstateType)
    if (onstateType) {
        if (onstateType == 'verifyWait') {
            filtersType.onstate = ['online'];
            filtersType.verifystate = [onstateType];
        }else if(onstateType == 'online'){
            filtersType.onstate = ['online'];
            filtersType.regstate = ['registered'];
            filtersType.verifystate = ['verifyPass'];
        } else {
