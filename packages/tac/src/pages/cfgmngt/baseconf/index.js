...
export default (props) => {
  ...
  const validatorFn = (value, callback) => {
    if (value) {
      ...
      if (isCheck.indexOf(false) != -1) {
        callback(`输入字母、数字、中文字符、'.'、'_'、'-'及其组合,多个以“,”分隔`)
      } else {
        callback()
      }
    } else {
  ...
  let boxList = [
    ...
    {
      value: "idCard",
      label: language('project.cfgmngt.baseconf.idcardclass')
    }
  ]
  ...
  const columns = [
    ...
    },
    {
      ...
      valueEnum: {
        ...
        idCard: {
          "id": "idCard",
          "text": language('project.cfgmngt.baseconf.idcardclass')
        },
      },
    },
    {
   ...
   * @param {*最小字节长度} minLen 
   */
  const validatorNameFn = (value, callback) => {
    let rules = [{
  ...
  return (<>
    ...
    <ModalForm {...modalFormLayout}
      ...
      <ProFormText 
        name='name' 
        label={language('project.cfgmngt.baseconf.fieldname')} 
        ...
        rules={[
          { required: true },
          {
            validator: (rule, value, callback) => {
              validatorNameFn(value, callback)
            }
          }
        ]}
       />
