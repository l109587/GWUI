...
import { EditFilled, UnorderedListOutlined, SaveFilled, ClusterOutlined, CloseOutlined, ApiFilled, LoadingOutlined, CopyFilled, EyeFilled } from '@ant-design/icons';
...
export default (props) => {
	...
	const copyRowJump = (text, record) => (
		<Popconfirm onConfirm={() => {
			console.log(record)
			history.push({ pathname: '/assmngt/applyrcd', state: { record: record } })
		}} key="copyrowjupm"
			title={language('assmngt.tempyrcd.jumptoapplicationentryporcess')}
			okButtonProps={{
				loading: confirmLoading,
			}} okText={language('project.yes')} cancelText={language('project.no')}>
			<a>{text}</a>
	...
	const columnsList = [
		...
		},
		{
			...
			filters: [
				{ text: language('assmngt.tempyrcd.expired'), value: 'expired' },
				{ text: language('project.assmngt.unsubmitted'), value: 'unsubmitted' },
				{ text: language('assmngt.tempyrcd.submitted'), value: 'submitted' },
				{ text: language('assmngt.tempyrcd.terminated'), value: 'terminated' },
			],
			render: (test, record, index) => {
				...
				} else if (record.orderState == 'terminated') {
					color = 'purple';
					text = language('assmngt.tempyrcd.terminated');
				} else if (record.orderState == 'expired') {
					color = 'error';
					text = language('assmngt.tempyrcd.expired');
				} else if (record.orderState == 'submitted') {
					color = 'success';
					text = language('assmngt.tempyrcd.submitted');
				} else {
      ...
      width: 180,
		...
		{
			title: language('assmngt.tempyrcd.valitime'),
			dataIndex: 'valiTime',
			width: 150,
			readonly: true,
			ellipsis: true,
					...
					<>
						<a key="editable"
							...
							<Tooltip title={language('project.see')} >
								<EyeFilled className='seeiconbox' size={18} style={{ fontSize: '18px' }} />
							</Tooltip>
						</a>
					</> : '',
				record.orderState == 'submitted' || record.orderState == 'expired' ?
					<>
						<a key="editable"
							onClick={() => {
								seeModalFrame(record, 'see');
						...
						{copyRowJump(<Tooltip title={language('project.copy')} ><CopyFilled size={18} style={{ fontSize: '18px', color: '#FF0000' }} /></Tooltip>, record)}
					</> : '',
			],
		},
	];
	...
	const [authSubmitStatus, setAuthSubmitStatus] = useState(false);
	//签章内容
	...
	const tableKey = 'atempryrcdrequest';//table 定义的key
	...
	const columnvalue = 'atempryrcdrequestcolumnvalue';//设置默认显示的 key 变动 set.strot 存储key
	const apishowurl = '/cfg.php?controller=confAssetManage&action=showTemporaryINApply';//接口路径
	const [queryVal, setQueryVal] = useState();//首个搜索框的值
	let searchVal = { queryVal: queryVal, queryType: 'fuzzy', showType: 'apply', flow: typeName, assetflow: assetflowsq };//顶部搜索框值 传入接口
	const onExpandUrl = '/cfg.php?controller=confAssetManage&action=showTemporaryINApply';
	...
	const delList = (record) => {
		let data = {};
		data.ids = record.id;
		post('/cfg.php?controller=confAssetManage&action=delTemporaryINApply', data).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	const save = (action, record = '', typeModal = '') => {
		...
		post('/cfg.php?controller=confAssetManage&action=setTemporaryINApply', data).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	//判断是否弹出文件签章model
	const getSaModal = (status, record = '', type = '') => {
		if (status == 1) {
			saModContent(record);
			if (type) {
	...
	//签章流程字段回显
	const signatureTemplate = async (record, type = '') => {
		if (type) {
			//是否是自动签章
			...
			if (res.success) {
				...
				post('/cfg.php?controller=confAssetManage&action=showSignINSingleApply', row).then((res) => {
					if (res?.data[0]) {
						...
						record.ipaddr = res?.data[0].ipaddr[0].ipaddr;
					}
					setRecordFind(record);
	...
	const seeModalFrame = (record, type) => {
		...
		if (record.batch == 'Y' && type == 'file') {
			post('/cfg.php?controller=confAssetManage&action=showTemporaryINApplyFlow', data).then((res) => {
	...
	const titleNameStep = (state) => {
		switch (state) {
			...
			case ('expire'):
				return language('assmngt.tempyrcd.expire');
			case ('delete'):
				return language('assmngt.tempyrcd.delete');
			case ('replace'):
				return language('assmngt.tempyrcd.replace');
			default:
				return language('assmngt.tempyrcd.retreatnetwork');
		}
	}
		...
		return approvalProcessList.map((item, index) => {
			...
			return (
				<>
					...
					{approvalNum == 1 ?
						<>
							<Step title={language('assmngt.tempyrcd.noexpire')}
								status='process'
					...
					{ index + 1 >= approvalNum && (item.result == 'expire' || item.result == 'delete' || item.result == 'replace') ?
						<Step title={item.result === 'expire' ? language('assmngt.tempyrcd.retreatnetwork') : language('assmngt.tempyrcd.termination')}
							icon={icon}
							status='process'
							description={
								<ProCard className='stepcard' bordered={true} direction='column' ghost >
									<div className='cardbox'>
										<div style={{ display: 'flex', alignItems: 'center' }}>
											...
											{item.result === 'expire' ? language('assmngt.tempyrcd.assetandresourcedatarecoveryfromdeviceside') : language('assmngt.tempyrcd.terminationcurrenttemporaryworkorder')}
										</div>
									</div>
									<div style={{ paddingLeft: 50 }}>
										<div  >{item.notes}</div>
	...
	return (<>
		...
		<DrawerForm
			...
			formRef={formRef}
			htmlType='submit'
			title={language('assmngt.tempyrcd.temporaryapplication')}
			...
			submitter={{
				render: (props, doms, info) => {
					return [
						...
						<Button key="buttonsubmit" type='primary' htmlType='submit'
						onClick={() => {
			...
			<div style={{ paddingLeft: 10 }}>
				<NotesText name="notes" width='443px' label={language('project.remark')} required={false} />
			</div>
		...
		>
			<div className='dynamicbox' style={{ marginLeft: '10px' }}>
				<Alert className='filealert' message={language('project.assmngt.assinput.uploadfilebuisusgprivatefield')} style={{ height: '100%' }} type="info" showIcon />
			...
			{recordFind.batch == 'Y' ?
				...
				<div className='sapplicationinformation'>
					...
					<ProDescriptions column={2}>
						...
						<ProDescriptions.Item name="vlan" label={language('assmngt.tempyrcd.locatedvlan')}>{initialValue.vlan ? initialValue.vlan : ' '}</ProDescriptions.Item>
					</ProDescriptions>
					<ProDescriptions column={2}>
				...
				<ProCard ghost style={{ height: '100%' }}>
					...
					<ProCard ghost direction='column' style={{ height: '272px', padding: '10px' }} bordered gutter={[3, 3]}>
						...
						<ProCard ghost style={{ height: '37px' }} >
							<Alert className='taddressalertinfo' message={language('project.assmngt.approval.addsalerttext')} style={{ height: '100%' }} type="info" showIcon />
						</ProCard >
						{/* 下层按钮盒子 */}
						<ProCard ghost  >
							<ProCard ghost  >
