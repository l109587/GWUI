...
import { EditFilled, ClusterOutlined, UnorderedListOutlined, CheckOutlined, CloseCircleOutlined, CloseOutlined, ApiFilled, LoadingOutlined, EyeFilled } from '@ant-design/icons';
import { ProTable, ModalForm, ProFormText, ProFormTextArea, ProFormSelect, ProCard, ProDescriptions, DrawerForm, ProForm, ProFormItem, ProFormSwitch } from '@ant-design/pro-components';
...
import { SignTable, BatchTemplate } from '@/common';
...
export default (props) => {
	...
	let columnsList = [
      ...
      width: 180,
			onCell: (record, rowIndex, key) => {
				if (record.changedFields?.indexOf('fullZone') != -1) {
					return { className: 'tabletdcolor' }
				}
			},
    },
	...
	let searchVal = { queryVal: queryVal, queryType: 'fuzzy', showType: 'apply', flow: typeName, assetflow: assetflow, buisUsg: buisUsgVal };//顶部搜索框值 传入接口
	let expandData = {};
	...
	const downloadClick = (list = {}) => {
		let api = '/cfg.php?controller=confAssetManage&action=exportChangeApply';
		let data = list;
 ...
 * */
	...
	//签章流程字段回显
	const signatureTemplate = (record, type = '') => {
		if (type) {
			//是否是自动签章
	...
	const fileSuccess = (evl) => {
		//审批通过
		approvalAdopt('auth', recordFind.batch == 'Y' ? 'batch' : '')
	}
	...
	const approvalAdopt = (values = '', type = '') => {
		if (values == 'auth') {
			...
			if (contractSign) {
				if (type == 'batch') {
					showExamineForm('closeAuth');
				} else {
					allApporvalModal();
				}
				signatureTemplate(recordFind, 'automatic');
			} else {
				let data = {};
				let obj = '';
				if (type == 'batch') {
	...
	return (<>
			...
			<div ref={pdfFormRef}>
				<div style={{ padding: '24px' }}>
					{isBatch == 'Y' ? (
						<BatchTemplate
							dataSource={topData}
							contractSig={contractSign ? true : false}
		...
		<DrawerForm
			...
			<div className='approvalprocess alfrommodalmargin'>
				...
				{recordFind.showSignature == 'Y' ?
					<ProDescriptions column={2}>
						<ProDescriptions.Item label={language('project.assmngt.resapply.atuomaticsignature')}>
							<ProFormSwitch
				...
				<NotesText
					...
					label={language('project.assmngt.resapply.approvedescription')}
					width='100%'
				/>
			</div>
		</DrawerForm>
			...
			<ProCard ghost >
				<ProCard ghost bodyStyle={{ padding: 0 }}>
					...
					<NotesText
						...
						label={language('project.assmngt.assapply.assspplytip')}
						width='280px'
					/>
				</ProCard>
				<ProCard ghost bodyStyle={{ padding: 0 }}>
