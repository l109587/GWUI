...
import { CheckOutlined, CloseCircleOutlined, EditFilled, UnorderedListOutlined, SaveFilled, ClusterOutlined, CloseOutlined, ApiFilled, LoadingOutlined, CodeSandboxCircleFilled, EyeFilled } from '@ant-design/icons';
import { ProTable, ProForm, ModalForm, ProFormText, ProFormTextArea, ProFormSelect, ProCard, ProDescriptions, DrawerForm, EditableProTable, ProFormDateTimePicker, ProFormRadio, ProFormItem, ProFormSwitch } from '@ant-design/pro-components';
...
import './index.less';
import PdfSeize from '@/assets/image/pdfseize.png'
import PdfBatchSeize from '@/assets/image/pdfbatchseize.png'
import Sigimg from '@/assets/image/signature.jpeg';
import $ from 'jquery'
import { TableLayout, PDFViewer, ExportPDF } from '@/components';
import { SignTable, BatchTemplate, DynFieldReg } from '@/common';
...
export default (props) => {
			...
			render: (text, record, _, action) => [
				<>
					<a key="editable" onClick={() => {
						let row = formRef.current.getFieldsValue(true);
						getMenu(false, row, record.subnet);
						getMaskModal(1);
					}}>
						<EditFilled />
					</a>
	...
	const columnsList = [
		...
		{
      title: language('project.assmngt.fullzone'),
      dataIndex: 'fullZone',
      ellipsis: true,
      width: 180,
    },
		...
		},
		{
			...
			render: (test, record, index) => {
				return <a onClick={() => {
					setOptionType('mod');
					setImEditRow(record);
					getMenu(false, record, record.ipstr);
					getMaskModal(1);
	...
	const [addSignatureType, setAddSignatureType] = useState('N');
	...
	const [authSignStatus, setAuthSignStatus] = useState(false);
	...
	let searchVal = { queryVal: queryVal, queryType: 'fuzzy', showType: 'apply', flow: typeName, assetflow: assetflowsq, buisUsg: buisUsgVal };//顶部搜索框值 传入接口
	...
	const tableTopSearch = () => {
		return (
			<Space className='asallocationbox'>
				<Search
	...
	const downloadClick = (list = {}) => {
		...
		DownnLoadFile(api, data, setFileDownLoading, true, '', true);
	}
	...
	//导入功能
	const handleEnter = async (option, record, val, list = '', typeFrom = false) => {
		console.log(record)
		let info = list ? list : upTableData
		let data = {}
		data.op = contractSign && typeFrom ? 'save' : option;
		data.list = JSON.stringify(info);
		data.flow = typeName;
		data.batch = 'Y';
		data.assetflow = assetflowsq;
		data.autosig = contractSign ? 'Y' : 'N';
		if (record) {
			data.id = record.id;
		}
		...
		if (!res.success) {
			if (option == 'submit' && contractSign && res.code == 2905) {
				showUpTable('close')
				let findData = {};
				findData.id = record.id;
				findData.showFirst = 'N';
				let row = await postAsync('/cfg.php?controller=confAssetManage&action=showSignINSingleApply', findData);
				let findRow = row.data[0];
				findRow.id = record.id;
				findRow.batch = 'Y';
				findRow.op = option;
				signatureTemplate(findRow, 'automatic');
				return false;
			}
	...
	const importTitle = async (info) => {
		...
		post('/cfg.php?controller=confAssetManage&action=importApply', data)
			.then((res) => {
				...
				getCloseImport();
				setTimeout(() => {
					getSignatureStatus(res.data[0].buisUsg, 'Y');
					setSpinning(false);
					showUpTable('open');
				}, 100)
			})
	 ...
	 * */
	const getDynamicField = (type = 'table', attribute = 'public', buisusg = '', record = {}, submitType = false) => {
		let data = {};
		...
		post('/cfg.php?controller=confResField&action=showResFieldList', data).then((res) => {
			if (res.data) {
				if (type == 'submit' || type == 'save' || type == 'tableSubmit') {
					...
					save(type == 'tableSubmit' ? 'submit' : type, record, type != 'tableSubmit' ? 'zfile' : '', submitType ? false : true);
				} else {
					if (attribute == 'public') {
	...
	const mod = (record) => {
		...
		setOrgValue(record.fullOrg);
		setApprovalList(record.ipaddr);
		setAddSignatureType(record.showSignature);
		setContractSign(record.autosig == 'Y' || record.autosig === true ? true : false);
		record.signatureStatus = record.showSignature == 'Y' || record.showSignature === true ? true : false;
	...
	//更新修改功能
	const save = async (action, record = '', typeModal = '', qzStatus = true) => {
		...
		data.ipaddr = record ? Array.isArray(record.ipaddr) ? JSON.stringify(record.ipaddr) : JSON.stringify([record.ipaddr]) : JSON.stringify(approvalList);
		data.user = obj.user;
		...
		if (record) {
			let findData = {};
			findData.id = data.id;
			findData.assetflow = assetflowsq;
			let row = await postAsync('/cfg.php?controller=confAssetManage&action=showSignINSingleApply', findData);
			authQz = row?.data[0]?.autosig;
			data.autosig = authQz;
		}
		let res = await postAsync('/cfg.php?controller=confAssetManage&action=setSignINApply', data);
		if (!res.success) {
			if (record && authQz == 'Y') {
				if (res.code == 2905) {
					data.batch = 'N';
					data.fullZone = data.zone;
					data.fullOrg = data.org;
					signatureTemplate(data, 'automatic', 'save');
					return false;
				}
			}
			message.error(res.msg);
		...
		if (typeModal == 'zfile') {
			...
			data.fullZone = data.zone;
			data.fullOrg = data.org;
			if (action == 'save' && contractSign && qzStatus) {
				signatureTemplate(data, 'automatic', 'save');
			}
		} else {
			closeSaModal(2);
	...
	//批量提交
	const plSave = (op = '', list, record = {}, qzStatus = false) => {
		console.log(qzStatus)
		console.log(record);
		let data = {}
		data.op = op
		data.id = record.id;
		...
		post(
			...
			if (!res.success) {
				record.op = op;
				if (res.code == 2905 && record.autosig == 'Y') {
					signatureTemplate(record, 'automatic');
					return false;
				}
				message.error(res.msg)
				return false
	...
	//判断是否弹出文件签章model
	const getSaModal = (status, record = '', type = false) => {
		console.log(type)
		if (status == 1) {
			saModContent(record);
			if (type) {
				setAuthSignStatus(true);
			}
	...
	//签章流程字段回显
	const signatureTemplate = async (record, type = false, showFrom = '') => {
		console.log(type)
		if (type) {
			//是否是自动签章
		...
		post('/cfg.php?controller=confSignature&action=showTemplate', data).then((res) => {
			if (res.success) {
				...
				if (record.batch == 'Y') {
					setRecordFind(record);
					getSaModal(1, record, type);
				} else {
					post('/cfg.php?controller=confAssetManage&action=showSignINSingleApply', row).then((res) => {
						if (res?.data[0]) {
							record.vlan = res?.data[0]?.vlan;
							record.ipaddr = res?.data[0]?.ipaddr[0];
							record.ipstr = res?.data[0]?.ipaddr[0]?.ipaddr;
							record.applicant = res?.data[0]?.applicant;
							record.zone = res?.data[0]?.zone;
							record.org = res?.data[0]?.org;
							record.company = res?.data[0]?.company;
							record.createTime = res?.data[0]?.createTime;
						}
						setRecordFind(record);
						getSaModal(1, record, type);
					}).catch(() => {
	 ...
	 * @param {*} record 
	 */
	const submitSignINApply = async (record, type = '') => {
		let data = {};
		data.id = record.id;
		data.orderID = record.orderID;
		data.assetflow = assetflowsq;
		let row = await postAsync('/cfg.php?controller=confAssetManage&action=showSignINSingleApply', data);
		let authQz = row?.data[0]?.autosig;
		let res = await postAsync('/cfg.php?controller=confAssetManage&action=submitSignINApply', data);
		if (!res.success) {
			if (!type && authQz == 'Y') {
				if (res.code == 2905) {
					signatureTemplate(record, 'automatic');
	 ...
	 */
	const showPSignINApply = (record, type = '', batchData = [], qzType = false) => {
		let data = {};
		data.id = record.id;
		data.orderID = record.orderID;
		...
		post('/cfg.php?controller=confAssetManage&action=showSignINSingleApply', data).then((res) => {
			if (type == 'template') {
				...
				upTableListData = res.data;
				setAddSignatureType(record.showSignature);
				setContractSign(record.autosig == 'Y' ? true : false);
				showUpTable('open');
			} else {
				plSave(type, res.data ? res.data : [], record, qzType);
			}
		}).catch(() => {
			console.log('mistake')
		})
	...
	// 自动分配功能 start
	// IP地址申请工单子网回显列表
	const getMenu = (type = false, record = '', aipAddr = false) => {
		let data = {};
		...
		post('/cfg.php?controller=confIPAddrManage&action=showAllocSubnet', data).then((res) => {
			...
			if (type) {
				...
				setSelectMenuId(xID === false ? res.data[0].id : xID)
				getBottonList(xID === false ? res.data[0].id : xID, record)
			}
		}).catch(() => {
			console.log('mistake')
		})
	...
	const approvalModalFrame = (type = 'applyfor', record = '', option = '') => {
		...
		if (type == 'editvlan') {
			...
			data.macaddr = record.macaddr;
			data.ipList = JSON.stringify(upTableListData.length > 0 ? upTableListData.map(v => { return v.ipstr }) : []);
		} else {
			let obj = formRef.current.getFieldsValue(['vlan', 'macaddr']);
			if (type == 'applyfor') {
				data.macaddr = obj.macaddr;
				data.orgID = orgVal;
			} else {
				data.macaddr = initialValue.macaddr;
		...
		if (type) {
			...
			if (recordFind.op) {
				if (recordFind.batch == 'Y') {
					showPSignINApply(recordFind, recordFind.op, [], true)
				} else {
					getDynamicField('tableSubmit', 'private', recordFind.buisUsg, recordFind, true);
				}
			} else {
				submitSignINApply(recordFind, 'approval');
			}
		}
		setIsmsgVisible(false);
	}
 ...
 */
	...
	const approvalAdopt = (value = '', type = '') => {
		...
		post('/cfg.php?controller=confAssetManage&action=agreeAssetApply', data).then((res) => {
			...
			if (contractSign) {
				signatureTemplate(initialValue, 'automatic');
			} else {
				setIncID(incID + 1);
			}
		}).catch(() => {
	...
	const getSignatureStatus = (buiSusg = '', isbatch = 'N') => {
		let data = {};
		data.name = typeName;
		data.isbatch = isbatch;
		data.buiSusg = buiSusg;
		post('/cfg.php?controller=confSignature&action=showAssetInSigState', data).then((res) => {
			if (res.success && res.data) {
				setAddSignatureType(res.data.showSignature);
			}
		}).catch(() => {
			console.log('mistake')
		})
	...
	return (<>
		...
		<DrawerForm
			...
			drawerProps={{
				className: 'closebuttonright apyaddmodalfrom allocationaddfrom',
				destroyOnClose: true,
				maskClosable: false,
				placement: 'right',
				onClose: () => {
			...
			{fpalertContent ? <Alert
				...
				style={{ width: 443, marginLeft: 10, height: '100%' }}
			...
			/> :
				<Alert className='aaddressalertinfo' message={language('project.assmngt.addrallc.fromzoneandvlanedit')} type="info" showIcon style={{ width: 443, marginLeft: 10, height: '100%' }} />
			}
			{addSignatureType == 'Y' ?
				<div style={{ paddingLeft: 10, paddingTop: '12px' }}>
					<div className='addsingswitchbox'><ProFormSwitch
						name='signatureStatus'
						label={language('project.assmngt.resapply.atuomaticsignature')}
						value={contractSign}
						onChange={(e) => {
							setContractSign(e);
						}}
						addonAfter={language('project.assmngt.resapply.automaticallysubmitlocaleletronicfiles')} />
					</div>
				</div>
				: <></>}
		...
		<DrawerForm
			...
			drawerProps={{
				className: 'asallocationuploadform allocationbatchmodfrom',
				placement: 'right',
				destroyOnClose: true,
				closable: false,
				maskClosable: false,
					...
					return [
						...
						<Button
							...
							onClick={() => {
								props.submit()
								// setAction()
								handleEnter('save', modPFromType ? recordFind : '', 'upTable', '', true)
							}}
						>
						...
						<Button
							...
							onClick={() => {
								props.submit()
								handleEnter('submit', modPFromType ? recordFind : '', 'upTable', '', true)
							}}
						>
		...
		>
			...
			/>
			{addSignatureType == 'Y' ?
				<div style={modPFromType ? { paddingTop: '12px' } : { paddingLeft: 24, paddingTop: '12px' }}>
					<div className='addsingswitchbox'><ProFormSwitch
						name='signatureStatus'
						label={language('project.assmngt.resapply.atuomaticsignature')}
						value={contractSign}
		...
		<DrawerForm
			...
			submitter={{
				render: (props, doms, info) => {
					return authSignStatus ? [
						...
						<Button key="buttonsave" type='primary' htmlType='submit'
							onClick={() => {
								let rowFind = { ...recordFind };
								rowFind.typeName = typeName;
								ExportPDF(signTitle, pdfFormRef.current, false, isBatch == 'Y' ? 'l' : 'p', '/cfg.php?controller=confSignature&action=uploadSignature', rowFind, fileSuccess, fileError);
							}}
						>
							<span className='buttonmargint'>{language('project.upload')}</span>
						</Button>,
					] : [
						doms[0],
						<Button key="buttonsave" type='primary' htmlType='submit'
			...
			submitTimeout={2000} >
			<div >
				<div ref={pdfFormRef} className='ccccccccccc'>
					<div style={{ padding: '24px' }}>
						{isBatch == 'Y' ?
							<BatchTemplate dataSource={topData} isSqOrder={recordFind.isSqOrder ? false : true} contractSig={contractSign ? true : false} bottomData={bottomData} editable={false} title={signTitle} signName={signName} dateTime={signDataTime} batchData={batchData} />
							: <SignTable dataSource={topData} isSqOrder={recordFind.isSqOrder ? false : true} contractSig={contractSign ? true : false} bottomData={bottomData} editable={false} title={signTitle} signName={signName} dateTime={signDataTime} />
						}
					</div>
				</div>
			</div>
		...
		<ModalForm
			...
			<ProCard className='abottomboxone' style={{ height: '272px' }} ghost gutter={[13, 13]}>
				<ProCard ghost style={{ height: '100%' }}>
					{/* 左侧列表 */}
					<ProCard ghost bordered colSpan="180px" style={{ height: '100%' }}>
		...
		<DrawerForm
			...
			<div style={{ backgroundColor: 'white', zIndex: 9 }}>
				...
				<div style={{ marginLeft: '57px' }}>
					<Alert className='caddressalertinfo' style={{ marginBottom: '12px', marginLeft: '-18px', marginRight: '20px' }} message={language('project.resmngt.assapply.scanthesigneddocumentspdffilesanduploadthem')} type="info" showIcon />
					<ProFormText name="vlan" initialValue={initialValue.vlan} width={'260px'} label={language('project.assmngt.wherevlan')} addonAfter={shDistribution} />
				</div>
				<div className='assignmentinformation alfrommodalmargin'>
					<ProFormText name='applyIPAddr' >
						<EditableProTable
				...
				<div className='approvalprocess alfrommodalmargin'>
					...
					{initialValue.showSignature == 'Y' ?
						<div className='singswitchbox'><ProFormSwitch
							name='switch'
							label={language('project.assmngt.resapply.atuomaticsignature')}
							onChange={(e) => {
								setContractSign(e);
						...
						: <></>}
					<div className='sptextbox' style={{ marginBottom: '12px' }}>
		...
		<Modal
			className='qzfilepromptbox'
			title={language('project.title')}
			visible={ismsgVisible}
			onOk={() => { msgmodalOk() }}
			onCancel={() => { msgmodalOk('cancel') }}
			maskClosable={false}
			keyboard={false}
		>
			<p>{language('project.assmngt.assapply.signfileuploadsuccesssubmitnot')}</p>
		</Modal>
	</>);
};