...
import { EditFilled, PlusOutlined, ReloadOutlined, SearchOutlined, EyeFilled } from '@ant-design/icons';
...
import { HandPaintedPlate, ViewGridList } from '@icon-park/react';
...
let H = document.body.clientHeight - 335
var clientHeight = H
const { Search } = Input
const { Step } = Steps;
export default (props) => {
	...
	const typeName = 'entry_withdrawal';
	...
	const seeModalFrame = (record, type = '') => {
		...
		post('/cfg.php?controller=confAssetManage&action=showSignOUTFlow', data).then((res) => {
			...
			if (record.batch == 'Y') {
				setSaFileList(res.data.recycleInfo.length >= 1 ? res.data.recycleInfo : []);
			} else {
	...
	const save = (action, record = '', typeModal = '') => {
		...
		if (record) {
			...
			data.id = record.id;
			data.clearInfo = JSON.stringify(clearInfo);
			let clearCon = {};
