...
import { EditFilled, ClusterOutlined, UnorderedListOutlined, CloseOutlined, ApiFilled, EyeFilled } from '@ant-design/icons';
import { ProTable, ModalForm, ProFormText, ProFormTextArea, ProFormSelect, ProCard, ProDescriptions, DrawerForm, ProForm } from '@ant-design/pro-components';
import { Button, Input, message, Space, Tag, Popconfirm, Divider, Steps, Alert, Tooltip, Menu } from 'antd';
...
import { regList, regMacList } from '@/utils/regExp';
...
import { Seal, ViewGridList, HandPaintedPlate } from '@icon-park/react';
...
export default (props) => {
					...
					return (
						<>
							<a
								...
								onClick={() => {
									if (record.orderState == 'unsubmitted') {
										signatureTemplate(record);//获取签章 表内容	
									}
							...
							<a
								...
								onClick={() => {
									if (record.orderState == 'unsubmitted') {
										setFileName(record.signature);
										getDynamicField('file', 'private', record.buisUsg ? record.buisUsg : '', record, uploadColumnsFileArr);
									}
								}}><i class="mdui-icon material-icons" style={{ fontSize: '18px', marginLeft: '8px' }}>&#xe415;</i></a>
	...
	const examineRef = useRef();//批量审批
	const typeName = 'entry_change';
	const assetFlow = 'ASSETFLOW_LR';
	...
	const [changeInfoBg, setChangeInfo] = useState({});
	const [fileDownLoading, setFileDownLoading] = useState(false);

	/** table组件 start */
	const rowKey = (record => record.id);//列表唯一值
	const tableHeight = clientHeight + 146;//列表高度
		...
		return (
			<Space className='netasearchbox'>
				...
				<ProFormSelect options={purposeList}
					name="deviceID"
					placeholder={language('project.assmngt.resapply.purpose')}
					onChange={(key) => {
						setBuisUsgVal(key);
 ...
 * */
			...
			if (res.data) {
				if (type == 'submit' || type == 'save' || type == 'tableSubmit') {
					...
					if (attribute == 'public') {
						...
						if (type == 'table') {
							tableList(type, res.data, columnsOld.length >= 1 ? columnsOld : columnsOldList, attribute);
						} else if (type == 'formtable') {
							tableList(type, res.data, columnsFileOld ? columnsFileOld: columnsFileOldList, attribute);
						} else {
							let arr = dynamicFieldList.length >= 1 ? dynamicFieldList : dynamicFieldListLet;
							...
							if (type == 'see' || type == 'file') {
								...
								setUploadColumnsBatch([...uploadColumnsSee.concat(arr)]);
	...
	const tableList = (type, data, list, attribute) => {
		...
		data.map((item) => {
			...
			columnsArr.splice(type == 'formtable' ? -1 : -3, 0, info);
		})
	...
	const save = (action, record = '', typeModal = '') => {
		...
		if (record == '') {
			if (data.id) {
				...
				data.zone = record ? record.zone : initialValue.zone;
				data.org = record ? record.org : initialValue.org;
				data.assetflow = assetFlow;
				data.flow = typeName;
	...
	const seeModalFrame = (record, type = '', changeArr = []) => {
		...
		post('/cfg.php?controller=confAssetManage&action=showSignChangeFlow', data).then((res) => {
			...
			setChangeInfo(res.data.changeInfo ? res.data.changeInfo : {});
	...
	return (<>
		...
		<DrawerForm className='arequestform'
			...
			{op == 'add' ? <Alert className='caddressalertinfo'
				message={language('project.assmngt.applyrcd.ipaddrtitlecontent')}
				type="info" showIcon
			/> : <Alert className='caddressalertinfo'
				message={language('project.assmngt.changeipaddrcontent', { ipaddr: initialValue.queryAddr })}
				description={language('project.assmngt.applyrcd.ipaddrdescription')}
				...
				<ProDescriptions column={1}>
					...
					{op == 'add' ? <ProDescriptions.Item className='savemodellist savemodeltextarea' name='applicant' label={language('project.assmngt.resapply.changeipaddr')}>
						<Search style={{ width: "100% !important" }}
							placeholder={language('project.assmngt.applyrcd.search')}
							onSearch={(queryVal) => {
		...
		<DrawerForm {...modalFormLayout}
			...
			drawerProps={{
				className: 'closebuttonright apychangefilemodal',
				destroyOnClose: true,
				maskClosable: false,
				placement: 'right',
				onClose: () => {
		...
		{/** 导入数据弹出框 批量编辑 */}
		<DrawerForm
			title={<div className="apychangeuploadtitle">{language('project.assmngt.approval.applychange')}</div>}
			width="1000px"
			formRef={upTableRef}
			visible={upTableSta}
			onVisibleChange={setUpTableSta}
			drawerProps={{
				className: 'apychangecationuploadform',
				placement: 'right',
				destroyOnClose: true,
				closable: false,
				maskClosable: false,
