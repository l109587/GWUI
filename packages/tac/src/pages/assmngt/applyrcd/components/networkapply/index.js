...
import { TableLayout, PDFViewer, ExportPDF, LeftTree, CardModal } from '@/components';
...
export default (props) => {
  ...
  const editForm = (values, type = '') => {
    setAddType(type ? true : false);
    showDraw('open')
  ...
  const setEnter = (option, record, val) => {
    ...
    if (addType) {
      data.isTemporary = 'Y';
      data.ipaddr = JSON.stringify(props.rowFind.ipaddr);
      data.vlan = props.rowFind.vlan;
    } else {
  ...
  return (
    <>
      ...
      >
        ...
        <NotesText style={{ marginLeft: 5 }} name="notes" label={language('project.remark')} disabled={addType} required={false} />
      </DrawerForm>
      {/* 导入抽屉 */}
      <DrawerForm
        {...modalFormLayout}
        ...
        drawerProps={{
          className: "afilemodal networkafrombody",
          placement: 'right',
          destroyOnClose: true,
          closable: false,
          maskClosable: false,
