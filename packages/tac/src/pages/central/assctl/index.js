...
import { ControlFilled, IdcardFilled, ExclamationCircleFilled, SafetyCertificateFilled, AppleFilled, LoadingOutlined } from '@ant-design/icons';
...
export default (props) => {
	...
	const columnsList = [
		...
		},
		{
			...
			ellipsis: true,
			render: (text, record, index) => {
				if(record.scheckResult == 'pass'){
					return (
						<SafetyCertificateFilled style={{ color: '#12C189', fontSize: '22px', marginLeft: '2px' }} />
					)
				}
				if(record.scheckResult == 'fail'){
					return (
						<Tooltip title={record.scheckDetail}>
							<ExclamationCircleFilled style={{ color: '#FF0000', fontSize: '22px', marginLeft: '2px' }} />
						</Tooltip>
					)
				}
			},
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=analyze&action=exportAssetsCtrlinfo';
    let data = list;
    data.zoneID = zoneID;
