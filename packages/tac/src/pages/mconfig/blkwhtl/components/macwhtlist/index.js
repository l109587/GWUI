...
export default () => {
	...
	const columns = [
		...
		{
			// title: '白名单地址内容',
			title: language('project.mconfig.blkwhtl.whtmaccontent'),
			dataIndex: 'addrlist',
			align: 'left',
			ellipsis: true,
			render: (text, record, index) => {
	...
	const syncLanList = (record, op) => {
		...
		data.id = record.id;
		data.op = op;
		post('/cfg.php?controller=confMACList&action=syncWhiteMACList', data).then((res) => {
			if (!res.success) {
	...
	const syncundosaveurl = '/cfg.php?controller=confMACList&action=syncWhiteMACList';//同步撤销接口
	const assocshowurl = '/cfg.php?controller=device&action=showCfgLinkDev';//设备列表接口路径
	const cfg_type_type = 'cfgMACWhite';//设备列表类型
	const tableKeyVal = 'whitedevm';//列表唯一key
	...
	const tableTopSearch = () => {
		return (
			<Search
				placeholder={language('project.mconfig.blkwhtl.whttablesearch')}
				style={{ width: 200 }}
				onSearch={(queryVal) => {
	...
	const statusSaveAll = (status) => {
		post('/cfg.php?controller=confMACList&action=enableWhiteMACList', { status: status }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	const statusSave = (record, checked) => {
		...
		let id = record.id;
		post('/cfg.php?controller=confMACList&action=enableWhiteMAC', { id: id, status: status }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	const save = (info) => {
		...
		post('/cfg.php?controller=confMACList&action=setWhiteMAC', data).then((res) => {
	...
	const delList = (selectedRowKeys) => {
		...
		post('/cfg.php?controller=confMACList&action=delWhiteMAC', { ids: ids }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
		...
		<div>
          ...
          [{
            ...
            message: regCommonList.notesText.alertText
				...
				<ProForm.Item label={language('project.mconfig.blkwhtl.whtmaccontent')} name="addrlistinfo" trigger="onValuesChange" rules={[{ required: true }]}>
					<EditableProTable
						scroll={{ y: 170 }}
						rowKey="id"
						toolBarRender={false}
