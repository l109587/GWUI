...
import { Input, message, Modal, Form, Switch, Popconfirm, Tag, Space, Tooltip } from 'antd';
...
import { regCommonList } from '@/common/regExp';
...
import { PolicyTable } from '@/general';
...
export default () => {
	...
	const columns = [
		...
		{
			// title: '白名单地址内容',
			title: language('project.mconfig.blkwhtl.blkmaccontent'),
			dataIndex: 'addrlist',
			align: 'left',
			ellipsis: true,
			render: (text, record, index) => {
	...
	const syncLanList = (record, op) => {
		...
		data.id = record.id;
		data.op = op;
		post('/cfg.php?controller=confMACList&action=syncBlackMACList', data).then((res) => {
			if (!res.success) {
	...
	const fromcolumns = [
		{
			...
			formItemProps: () => {
				return {
					rules: [{ required: true, pattern: regMacList.mac.regex, message: regMacList.mac.alertText }],
				};
			},
		},
		{
	...
	const syncundosaveurl = '/cfg.php?controller=confMACList&action=syncBlackMACList';//同步撤销接口
	const assocshowurl = '/cfg.php?controller=device&action=showCfgLinkDev';//设备列表接口路径
	const cfg_type_type = 'cfgMACBlack';//设备列表类型
	const tableKeyVal = 'whitedevmb';//列表唯一key
	...
	const tableKey = 'macblk';//table 定义的key
	...
	const columnvalue = 'macblkcolumnvalue';//设置默认显示的 key 变动 set.strot 存储key
	const apishowurl = '/cfg.php?controller=confMACList&action=showBlackMACList';//接口路径
	...
	const tableTopSearch = () => {
		return (
			<Search
				placeholder={language('project.mconfig.blkwhtl.blktablesearch')}
				style={{ width: 200 }}
				onSearch={(queryVal) => {
	...
	const statusSaveAll = (status) => {
		post('/cfg.php?controller=confMACList&action=enableBlackMACList', { status: status }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	const statusSave = (record, checked) => {
		...
		let id = record.id;
		post('/cfg.php?controller=confMACList&action=enableBlackMAC', { id: id, status: status }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	const save = (info) => {
		...
		post('/cfg.php?controller=confMACList&action=setBlackMAC', data).then((res) => {
	...
	const delList = (selectedRowKeys) => {
		...
		post('/cfg.php?controller=confMACList&action=delBlackMAC', { ids: ids }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	return (
		<div>
			...
			<ModalForm {...modalFormLayout}
				...
				<NotesText name="notes" label={language('project.remark')} required={false}  rules={
          [{
            required: false,
            pattern: regCommonList.notesText.regex,
            message: regCommonList.notesText.alertText
          }]}/>
				<ProForm.Item label={language('project.mconfig.blkwhtl.blkmaccontent')} name="addrlistinfo" trigger="onValuesChange" rules={[{ required: true }]}>
					<EditableProTable
						scroll={{ y: 170 }}
						rowKey="id"
						toolBarRender={false}
