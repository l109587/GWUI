...
export default () => {
	...
	const columns = [
		...
		{
			// title: '白名单地址内容',
			title: language('project.mconfig.blkwhtl.servercontent'),
			dataIndex: 'addrlist',
			align: 'left',
			ellipsis: true,
			render: (text, record, index) => {
	...
	const syncLanList = (record, op) => {
		...
		data.id = record.id;
		data.op = op;
		post('/cfg.php?controller=confServerList&action=syncServerWhiteList', data).then((res) => {
			if (!res.success) {
	...
	const syncundosaveurl = '/cfg.php?controller=confServerList&action=syncServerWhiteList';//同步撤销接口
	const assocshowurl = '/cfg.php?controller=device&action=showCfgLinkDev';//设备列表接口路径
	const cfg_type_type = 'cfgServerWhite';//设备列表类型
	const tableKeyVal = 'whitedevs';//列表唯一key
	...
	const tableKey = 'serverwhite';//table 定义的key
	...
	const columnvalue = 'serverwhitecolumnvalue';//设置默认显示的 key 变动 set.strot 存储key
	const apishowurl = '/cfg.php?controller=confServerList&action=showServerList';//接口路径
	...
	const tableTopSearch = () => {
		return (
			<Search
				placeholder={language('project.mconfig.blkwhtl.servertablesearch')}
				style={{ width: 200 }}
				onSearch={(queryVal) => {
	...
	const statusSave = (record, checked) => {
		...
		let id = record.id;
		post('/cfg.php?controller=confServerList&action=enableServerWhiteIP', { id: id, status: status }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	const save = (info) => {
		...
		post('/cfg.php?controller=confServerList&action=setServerWhiteIP', data).then((res) => {
	...
	const delList = (selectedRowKeys) => {
		...
		post('/cfg.php?controller=confServerList&action=delServerWhiteIP', { ids: ids }).then((res) => {
			if (!res.success) {
				message.error(res.msg);
				return false;
			}
	...
	return (
		<div>
			...
			<ModalForm {...modalFormLayout}
				...
				<ProForm.Item label={language('project.mconfig.blkwhtl.servercontent')} name="addrlistinfo" trigger="onValuesChange" rules={[{ required: true }]}>
					<EditableProTable
						scroll={{ y: 170 }}
						rowKey="id"
						toolBarRender={false}
