...
import { UnorderedListOutlined, FileOutlined, IdcardFilled, ClusterOutlined, InfoCircleFilled, DeleteFilled, EditFilled, UploadOutlined, LoadingOutlined, FlagFilled } from '@ant-design/icons';
...
import { DynFieldReg } from '@/common';
...
export default () => {
    ...
    const [subnetsID, setSubnetsID] = useState(0);
    ...
    const tableHeight = cardHeightTop - 140;//列表高度
    const tableKey = 'readdrallc';//table 定义的key
    ...
    const columnvalue = 'readdrallccolumnvalue';//设置默认显示的 key 变动 set.strot 存储key
    const apishowurl = '/cfg.php?controller=confIPAddrManage&action=showIPSubNetAddr';//接口路径
    const typeLeft = true;
    const showPage = true;
    const [queryVal, setQueryVal] = useState();//首个搜索框的值
    let searchVal = { queryVal: queryVal, queryType: 'fuzzy', orgID: orgId };//顶部搜索框值 传入接口
    const incAdd = () => {
        let inc;
        clearTimeout(inc);
        inc = setTimeout(() => {
    ...
    const downloadClick = (list = {}) => {
        let api = '/cfg.php?controller=confIPAddrManage&action=exportIPAddr';
        let data = list;
        DownnLoadFile(api, data, setFileDownLoading)
    }

    const tableShowList = (res) => {
        closeFootListBox();
        if (res.data.length < 1) {
            ...
            if (subnetsID == 0) {
                ...
                res.data.map((item) => {
                    if (item.id == subnetsID) {
                        getBottonList(subnetsID, item);
                    }
                })
            }
        }
    ...
    return (
        <>
                ...
                <ProCard direction='column' ghost gutter={[13, 13]}>
                    <ProCard ghost gutter={[13, 13]}>
                        ...
                        <ProCard colSpan='calc(100% - 238px)' ghost style={{ height: cardHeightTop, backgroundColor: 'white' }}>
                            <ProtableModule typeLeft={typeLeft} tableShowList={tableShowList} concealColumns={concealColumns} columns={columns} apishowurl={apishowurl} incID={incID} showPage={showPage} clientHeight={tableHeight} columnvalue={columnvalue} tableKey={tableKey} searchText={tableTopSearch()} searchVal={searchVal} rowkey={rowKey} delButton={delButton} addButton={addButton} rowSelection={rowSelection} uploadButton={uploadButton} uploadClick={uploadClick} downloadButton={downloadButton} downLoadType={downLoadType} downloadClick={downloadClick} />
                        </ProCard>
            ...
            <DrawerForm   {...drawFormLayout}
                ...
                {leftOperation == 1 ?
                    (<> <Button icon={<EditFilled />} disabled={!writable} style={{ left: '48%' }} type="primary" onClick={() => {
                        setSubmitType('ipChange');
                        formRef.current.submit();
                    }}>
                        {language('project.resmngt.addrallc.change')}
                    </Button>
                        <Button icon={<DeleteFilled />} disabled={!writable} style={{ left: '52%' }} danger type="primary" onClick={() => {
                            setSubmitType('ipRecovery');
                            formRef.current.submit();
                        }}>
                            {language('project.resmngt.addrallc.recovery')}
                        </Button></>
                    ) : (<Button icon={<IdcardFilled />} disabled={!writable} style={{ left: '64%' }} type="primary" onClick={() => {
                ...
                <ProCard ghost gutter={[13, 13]}>
                    ...
                    <ProCard ghost colSpan='280px' >
                        <div style={{ marginTop: 250 }}>
                            <Button icon={<IdcardFilled />}  disabled={sOperationStatus === 'assigned' || !writable ? true : false} key='operation1' type="primary" onClick={() => {
                                setSubmitType('batchDistribution');
                                formRef.current.submit();
                            }}>
                                {language('project.resmngt.addrallc.distribution')}
                            </Button>
                            <Button style={{ marginLeft: 10 }} disabled={sOperationStatus === 'unassigned' || !writable ? true : false} key='operation2' icon={<EditFilled />} type="primary" onClick={() => {
                                setSubmitType('batchChange')
                                formRef.current.submit();
                            }}>
                                {language('project.resmngt.addrallc.change')}
                            </Button>
                            <Button style={{ marginLeft: 10 }} disabled={sOperationStatus === 'unassigned' || !writable ? true : false} key='operation3' icon={<DeleteFilled />} danger type="primary" onClick={() => {
                                batchIpRecovery();
                            }}>
                                {language('project.resmngt.addrallc.recovery')}
                            </Button>
