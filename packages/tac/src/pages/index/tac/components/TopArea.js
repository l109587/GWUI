...
import Checked from '@/assets/image/if-checked.svg';
import '../index.less';
  ...
  const showDevRegStats = () => {
    post('/cfg.php?controller=sysHeader&action=showDevRegStats').then((res) => {
      if (res.title) {
        ...
        res.data.ucount = (Math.floor((parseInt(res.data.regnum) / parseInt(res.data.total)) * 100) ? Math.floor((parseInt(res.data.regnum) / parseInt(res.data.total)) * 100) : 0) + '%';
