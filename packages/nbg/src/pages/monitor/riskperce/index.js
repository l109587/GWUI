  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <Col offset={6} className="ipRangeDiv">
          <div 
            className={formRef?.current?.getFieldsValue(["assetRange"]).assetRange == "1" && dataSource.length < 1 ? "" : "tableNoError"}
            style={{ marginLeft: 24, width: "420px" }}
          >
            <ProFormText name={"ipRange"} rules={[
              {
                required: formRef?.current?.getFieldsValue(["assetRange"]).assetRange == "1" && dataSource.length < 1 ? true : false,
                message: language('project.mandatory')
              },
            ]}>
                <EditTable
