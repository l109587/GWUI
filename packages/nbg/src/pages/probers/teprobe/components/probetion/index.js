...
const Configuration = (props) => {
  ...
  const clientHeight = contentHeight - 78;
  ...
  const [incID, setIncID] = useState(0) //递增的id 删除/添加的时候增加触发刷新
	...
	const [outliVal, setOutliVal] = useState(['']);
	...
	const getFormdata = (outlineVal,portVal) => {
		post('/cfg.php?controller=probeManage&action=showAgenCfg').then((res) => {
			...
			if (res.selfCheck == 'N') {
				...
				delete res["selfCheck"];
			}
			if (res.outlineBlock == 'N') {
				delete res["outlineBlock"];
			}
			...
			setOutliVal(obj.outlineCfgName)
	...
	/* 设置 */
	const setForm = () => {
		let obj = formRef.current.getFieldsValue(['outlineCfgName', 'monitorItems', 'IPV6AssetRpt','selfCheck', 'outlineBlock', 'period', 'port', 'exception']);
    ...
    data.outlineCfgName = obj.outlineCfgName;
		data.monitorItems = obj.monitorItems;
		data.IPV6AssetRpt = obj.IPV6AssetRpt ? 'Y' : 'N';
		data.selfCheck = obj.selfCheck ? 'Y' : 'N';
		data.outlineBlock = obj.outlineBlock ? 'Y' : 'N';
  ...
  let exportTimer = ''
	const getMakeStatus = (action) => {
		setMaking(true);
		post('/cfg.php?controller=probeManage&action=agentMakeOperate', {op: action}).then((res) => {
			if (res.code === 202) {
	...
	return (
		<div className='teprobeContent'>
			<Spin size="large" spinning={making} tip={language('probers.teprobe.probetion.probeMakeing')}>
				<Spin size="large" spinning={loading} tip={tipId === 1 ? language('project.isuploading') : ''}>
					<ProCard 
					  ghost 
						className='probetionCard' 
						bodyStyle={{ height: clientHeight, overflowY: "auto" }} 
					>
						<ProForm {...formleftLayout}
							...
							<ProFormSelect 
								...
								onChange={(value) => {
									if (value.length > 3) {
										value?.pop()
										message.error(language('probers.teprobe.outlineCfgName.multiple.nax'))
									}
									setOutliVal(value)
								}} 
							/>
							<div className='probetionmon'>
								<ProFormCheckbox.Group  id="aaa" name="monitorItems"
									onChange={(key, val) => {
									}}
							...
							<div className='IpV6RptDiv'>
								<ProFormCheckbox 
									label={language('probers.teprobe.IPv6mon')} 
									name='IPV6AssetRpt'
								>
									{language('probers.teprobe.IPv6monlabel')}
								</ProFormCheckbox>
							</div>
							<div className='selfCheck'>
								<ProFormCheckbox 
									label={language('probers.teprobe.probetion.selfCheck')} 
									name='selfCheck'
									onChange={(e) => {
										console.log(e)
										if (e.target.checked === true) {
											...
											formRef.current.resetFields(['outlineBlock']);
											setBlockDisable(true);
										}
									}}
								>
									{language('probers.teprobe.probetion.selfCheckText')}
								</ProFormCheckbox>
							</div>
							<div className='outBlock'>
								...
								>
									{language('probers.teprobe.probetion.outlineBlockText')}
								</ProFormCheckbox>
							</div>
							<div className='periodDIv'>
								<ProFormDigit name="period" width="260px" label={language('project.temporary.terminal.period')} fieldProps={{ precision: 0, controls: false, }} addonAfter={hourlaver} rules={[
									{
										required: true,
									...
									},
									{
										validator: (rule, value, callback) => {
											valiCompare(value, callback, 1, 60)
										}
									}
								]} />
							</div>

							<ProFormSelect width="300px"  name="port" label={language('project.temporary.terminal.port')} options={portselist} rules={[
								{
									required: true,
									message: language('project.messageselect')
								}
						...
						<div className="hdpbconTable">
							<ProtableModule
								...
								clientHeight={store.get('layout')==='top'? 229 : 189}
								tableKey={'probetion'}
