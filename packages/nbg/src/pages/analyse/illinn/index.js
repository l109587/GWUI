...
export default () => {
  ...
  const clientHeight = contentHeight - 375;
        ...
        return (
          <Tooltip title={record.os} placement="topLeft">
            <div style={{ display: "flex", alignItems: "center" }}>
              <div className="typeIcon">{OSIcon(record.os)}</div>
        ...
        if (record.info.type == "netIn") {
          return (
            <div className="infobox">
              ...
              <Tag color="cyan" style={{ width:67 }} >{record.info.basic}</Tag>
              {record.info.detail != "" ? (
                <Tooltip
                  getPopupContainer={() => document.getElementById("infoID")}
      ...
      .then((res) => {
        ...
        columnslist.map((item) => {
          if (item.dataIndex == "type") {
            ...
            item.filteredValue = info?.type ? info?.type : null;
          } else if (item.dataIndex == "online") {
            item.filters = onlinefillter;
            item.filterMultiple = false;
            item.filteredValue = info?.online ? info?.online : null;
          } else {
          }
        });
        setCols([...columnslist]);
  ...
  const delList = (selectedRowKeys, dataList, selectedRows) => {
    ...
    post("/cfg.php?controller=assetMapping&action=delVioInline", {
      delValue: arr.join(","),
    })
      .then((res) => {
        if (!res.success) {
