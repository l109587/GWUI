...
import { ExclamationCircleOutlined, InfoCircleOutlined, DeleteOutlined } from '@ant-design/icons';
...
const Behareport = () => {
  ...
  const module = 'audit_abnormal_whitelist';

  const apishowurl = '/cfg.php?controller=confPolicy&action=show';
  const columnvalue = 'abnormalbehaviorColumnvalue';
  const tableKey = 'abnormalbehavior';
  ...
  const handleSave = (values) => {
    ...
    delete values.share;
    let info = {};
    info.audit_type = values.audit_type;
    delete values.audit_type;
    info.expire_time = values.expire_time;
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.behavioradt.abnormalbehavior')}
      visible={modalStatus} onVisibleChange={setModalStatus} width='620px'
      modalProps={{
        maskClosable: false,
    ...
    />
    <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='audit_abnormal_whitelist' type='policy'/>
  </>)
}
