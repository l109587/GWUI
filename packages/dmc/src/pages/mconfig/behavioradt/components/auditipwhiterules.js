...
const Ipaudit = () => {
  ...
  const module = 'audit_ip_whitelist';
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportWhiteip';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importWhiteip';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importWhiteip';
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.behavioradt.auditipwhiterules')}
      ...
      <ProFormText label={language('dmcoconfig.behavioradt.ip')} name='ip'
        rules={[
          {
    ...
    />
    <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='audit_ip_whitelist' type='policy'/>
  </>)
}
