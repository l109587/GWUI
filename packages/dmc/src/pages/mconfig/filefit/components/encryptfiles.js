...
export default () => {
  ...
  const columnvalue = 'fencryptionFilterColunmval';
  const tableKey = 'encryptionFilter';
  const module = 'encryption_filter';
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportFileencry';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importFileencry';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importFileencry';
  ...
  return (
    <>
      ...
      <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
        autoFocusFirstInput title={language('dmcoconfig.filefit.encryptedfilefilterrules')}
      ...
      />
      <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='encryption_filter' type='policy'/>
    </>
  )
