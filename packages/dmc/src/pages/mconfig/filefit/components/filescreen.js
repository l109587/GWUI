...
const Encryption = () => {
  ...
  const columnlist = [
    ...
    {
      title: language('dmcoconfig.filefit.applicationlayerprotocol'),
      ...
      render: (text, record, index) => {
        let title = [];
        record.info?.app_protocol?.map(itemVal => {
          appProtocolList.map((item) => {
    ...
    {
      title: language('dmcoconfig.filefit.filename'),
      dataIndex: 'filename',
      key: 'filename',
      align: 'left',
      ellipsis: true,
  ...
  const [appProtocolData, setAppProtocolData] = useState([]);
  let appProtocolList = [];
  ...
  const tableKey = 'filescreen';
  const components = true;
  const columnvalue = 'filescreenColunmval';
  ...
  const getSelectData = () => {
    post('/cfg.php?controller=confPolicy&action=showData', { module: module }).then((res) => {
      ...
      setAppProtocolData(res?.data?.app_protocol)
      appProtocolList = res?.data?.app_protocol ? res?.data?.app_protocol : [];
      let appProtocolFilter = res?.data?.app_protocol?.map(item => ({
        text: item.label,
        value: item.value
      }));
      ...
      columnlist.map((item, index) => {
        ...
        } else if (item.dataIndex == "app_protocol") {
          item.filters = appProtocolFilter;
          item.filterMultiple = false;
        }
      })
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.filefit.filepropertyfilterrules')}
      ...
      <ProFormSelect
        label={language('dmcoconfig.filefit.applicationlayerprotocol')}
        name='app_protocol'
        options={appProtocolData}
        fieldProps={{
          mode: 'multiple'
        }}
      />
      <ProFormText name='filename' label={language('dmcoconfig.filefit.filename')} />
