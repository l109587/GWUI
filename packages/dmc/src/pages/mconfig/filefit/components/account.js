...
export default () => {
  ...
  const columnvalue = 'faccountFilterColunmval';
  const tableKey = 'accountFilter';
  const module = 'account_filter';
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportFileacco';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importFileacco';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importFileacco';
  ...
  return (
    <>
      ...
      <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
        autoFocusFirstInput title={language('dmcoconfig.filefit.accountscreeningrules')}
      ...
      />
      <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='account_filter' type='policy'/>
    </>
  )
