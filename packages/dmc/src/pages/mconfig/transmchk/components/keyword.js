...
const Keyword = () => {
  ...
  const columnlist = [
    ...
    {
      title: language('dmcoconfig.transmchk.min_match_count'),
  ...
  const columnvalue = 'keywordColunmval';
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportSensikey';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importSensikey';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importSensikey';
  ...
  return (
    <>
      ...
      <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
        autoFocusFirstInput title={language('dmcoconfig.transmchk.keywordteringrules')}
        ...
        <ProFormText name='min_match_count' label={language('dmcoconfig.transmchk.min_match_count')}
          rules={[
            {
              required: true
            }
      ...
      />
      <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='keyword_detect' type='policy'/>
    </>
  )
