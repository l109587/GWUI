...
const Picture = () => {
  ...
  const tableKey = 'Picture';
  const components = true;
  const columnvalue = 'pictureColunmval';
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.transmchk.picture.modaltitle')}
      ...
      <div className='flexDiv'>
        <ProFormText name='minsize' width={210} label={language('dmcoconfig.transmchk.minsize')} addonAfter={afterDiv('KB')}  rules={[{
          required: true,
          message: language('project.mandatory')
      }]} />
        <ProFormText name='maxsize' width={210} label={language('dmcoconfig.transmchk.maxsize')} addonAfter={afterDiv('KB')}  rules={[{
          required: true,
          message: language('project.mandatory')
      }]} />
      </div>
