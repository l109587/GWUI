...
const Keyword = () => {
  ...
  const columnlist = [
    ...
    {
      title: language('dmcoconfig.transmchk.sealtype'),
      dataIndex: 'seal_type',
      key: 'seal_type',
      align: 'center',
      ellipsis: true,
      ...
      render: (text, record, index) => {
        ...
        sealTypeList.map((item) => {
          if (record.info.seal_type == item.value) {
            title = item.label;
          }
        })
        if (title) {
  ...
  const [sealTypeDate, setSealTypeDate] = useState([]);
  let sealTypeList = [];
  /* 页面表格传参区 */
  const components = true;
  const columnvalue = 'tsealColunmval';
  const tableKey = 'tseal';
  const module = 'file_seal';
  ...
  const getSelectData = () => {
    post('/cfg.php?controller=confPolicy&action=showData', { module: module }).then((res) => {
      ...
      setSealTypeDate(res?.data?.seal_type);
      sealTypeList = res?.data?.seal_type ? res?.data?.seal_type : [];
      ...
      let sealTypeFillter = res?.data?.seal_type?.map(item => ({
        text: item.label,
        value: item.value
      }));
      columnlist.map((item, index) => {
        if (item.dataIndex == 'risk') {
          ...
          item.filterMultiple = false;
        } else if (item.dataIndex == "seal_type") {
          item.filters = sealTypeFillter;
          item.filterMultiple = false;
        }
      })
  ...
  return (
    <>
      ...
      <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
        autoFocusFirstInput title={language('dmcoconfig.transmchk.sealfilefilteringrules')}
        ...
        <ProFormSelect name='seal_type' label={language('dmcoconfig.transmchk.sealtype')} options={sealTypeDate}/>
