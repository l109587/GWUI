...
const Troation = () => {
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportAttaBlackdoma';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importAttaBlackdoma';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importAttaBlackdoma';
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.attachck.blacklist.domainblacklist')}
    ...
    />
    <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='domain_blacklist' type='policy'/>
  </>)
}
