...
const Troation = () => {
  ...
  const columnlist = [
    ...
    {
      title: language('dmcoconfig.attachck.blacklist.fromsip'),
      dataIndex: 'sip',
      key: 'sip',
      align: 'left',
      ellipsis: true,
    ...
    {
      title: language('dmcoconfig.attachck.blacklist.sport'),
      dataIndex: 'sport',
      key: 'sport',
      align: 'left',
      ellipsis: true,
    ...
    {
      title: language('dmcoconfig.attachck.blacklist.dip'),
      dataIndex: 'dip',
      key: 'dip',
      align: 'left',
      ellipsis: true,
    ...
    {
      title: language('dmcoconfig.attachck.blacklist.dport'),
      dataIndex: 'dport',
      key: 'dport',
      align: 'left',
      ellipsis: true,
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        protocolList.map((item) => {
          if (record.info.protocol == item.value) {
            title = item.label;
          }
        })
        if (title) {
  ...
  const columnvalue = 'atipblacklistColumnvalue';
  const tableKey = 'atipblacklist';
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportAttaBlackip';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importAttaBlackip';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importAttaBlackip';
  ...
  const getSelectData = () => {
    post('/cfg.php?controller=confPolicy&action=showData', { module: module }).then((res) => {
      ...
      protocolList = res?.data?.protocol ? res?.data?.protocol : [];
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.attachck.blacklist.ipblacklist')}
      ...
      <div>
        ...
        <ProFormText
          label={language('dmcoconfig.attachck.blacklist.sport')}
        ...
        <ProFormText
          label={language('dmcoconfig.attachck.blacklist.dport')}
          style={{ marginTop: '5px', marginRight: '5px' }}
          name='dport'
          rules={[{
            pattern: regPortList.portorrange.regex,
        ...
        <div className='protocolDiv'>
          <ProFormRadio.Group label={language('dmcoconfig.attachck.blacklist.protocol')} name='protocol' radioType='button' options={protocolData} fieldProps={{ buttonStyle: "solid" }} />
        </div>
      </div>
    ...
    />
    <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='ip_blacklist' type='policy'/>
  </>)
}
