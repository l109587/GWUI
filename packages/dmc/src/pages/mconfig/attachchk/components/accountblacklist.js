...
const Troation = () => {
  const module = 'account_blacklist';
  ...
  const columnlist = [
    ...
    {
      title: language('dmcoconfig.attachck.blacklist.accounttype'),
      ...
      render: (text, record, index) => {
        ...
        accountTypeList.map((item) => {
          if (record.info.account_type == item.value) {
            title = item.label;
          }
        })
        if (title) {
    ...
    {
      title: language('dmcoconfig.attachck.blacklist.account'),
      dataIndex: 'account',
      key: 'account',
      align: 'left',
      ellipsis: true,
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportAttaBlackacco';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importAttaBlackacco';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importAttaBlackacco';
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.attachck.blacklist.accountblacklist')}
    ...
    />
    <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='account_blacklist' type='policy'/>
  </>)
}
