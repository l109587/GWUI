...
import { ModalForm, ProCard, ProForm, ProFormText, ProFormSwitch, ProFormSelect, ProFormDigit, ProFormTextArea } from '@ant-design/pro-components';
...
const Troation = () => {
  const module = 'alarm_hash_whitelist';
  let ruletypeDate = [
    {
      label: 'md5',
      text: 'md5',
  ...
  const downloadClick = (list = {}) => {
    let api = '/cfg.php?controller=confPolicy&action=exportAttaWhitehash';
    let data = list;
  ...
  const uploadUrl = '/cfg.php?controller=confPolicy&action=importAttaWhitehash';
  const uploadAddUrl = '/cfg.php?controller=confPolicy&action=importAttaWhitehash';
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.attachack.whitelist.filehashwhiterules')}
    ...
    />
    <Policy visible={visible} setVisible={setVisible} operate={operate} rowInfo = {rowRecord} module='alarm_hash_whitelist' type='policy'/>
  </>)
}
