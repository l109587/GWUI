...
const Url = () => {
  const columnlist = [
    ...
    {
      title: language('dmcoconfig.targetadt.urlinfo'),
      dataIndex: 'url',
      key: 'url',
      align: 'left',
      ellipsis: true,
  ...
  const columnvalue = 'UrlColunmval';
  ...
  const tableTopSearch = () => {
    return [
      <Search allowClear placeholder={language('dmcoconfig.targetadt.url.searchtext')}
        onSearch={(queryVal) => {
  ...
  return(<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.targetadt.url.modaltitle')}
      ...
      <ProFormText label={language('dmcoconfig.targetadt.urlinfo')} name='url' rules={[{
        required: true,
