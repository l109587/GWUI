...
import { Tooltip, Input, DatePicker, Space, message } from 'antd'
...
export default () => {
  ...
  const tableKey = 'evtdevice'
  const apishowurl = '/cfg.php?controller=evtDevice&action=showEvtDeviceList'
  ...
  const getChartData = () => {
    post('/cfg.php?controller=evtDevice&action=showEvtDeviceChart', {
      begDate: olddata,
      endDate: newdata,
    }).then((res) => {
      if (!res.success) {
    ...
    <ProCard direction="column" ghost gutter={[16, 16]}>
      ...
      <ProCard ghost>
        <ProtableModule
          ...
          columnvalue={'evtdevicecolumnvalue'}
        />
      </ProCard>
    </ProCard>
  )
