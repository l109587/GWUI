...
export default () => {
  const columns = [
    {
      title: language("ecpmngt.blkname"),
      dataIndex: "name",
      align: "left",
      fixed: "left",
      ellipsis: true,
    ...
    },
    {
      ...
      render: (text, record, index) => {
        if (record.macaddr) {
          let addrlist = record.macaddr;
          return (
            <>
              <CutdropDown addrlist={addrlist} />
            </>
          );
        }
      },
  ...
  const tableKey = "emacblk"; //table 定义的key
  ...
  const columnvalue = "emacblkcolumnvalue"; //设置默认显示的 key 变动 set.strot 存储key
  const apishowurl = "/cfg.php?controller=exceptDev&action=get_macblack"; //接口路径
  ...
  const tableTopSearch = () => {
    return (
      <Search
        placeholder={language("ecpmngt.macblk.tablesearch")}
        style={{ width: 200 }}
        onSearch={(queryVal) => {
  ...
  const save = (info) => {
    ...
    let url = "/cfg.php?controller=exceptDev&action=add_macblack";
    if (op == "mod") {
      url = "/cfg.php?controller=exceptDev&action=modify_macblack";
    }
    post(url, data)
      .then((res) => {
        if (!res.success) {
  ...
  const delList = (selectedRowKeys) => {
    ...
    post("/cfg.php?controller=exceptDev&action=del_macblack", { ids: ids })
      .then((res) => {
        if (!res.success) {
          message.error(res.msg);
          return false;
  ...
  return (
    ...
    >
      ...
      <DrawerForm
        ...
        autoFocusFirstInput
        drawerProps={{
          className: "uploadmacblkdrawfrom",
          destroyOnClose: true,
          maskClosable: false,
          placement: "right",
          getContainer: false,
