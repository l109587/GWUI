...
export default () => {
  ...
  const tableKey = "eipwht"; //table 定义的key
  ...
  const columnvalue = "eipwhtcolumnvalue"; //设置默认显示的 key 变动 set.strot 存储key
  const apishowurl = "/cfg.php?controller=exceptDev&action=get_unmanage"; //接口路径
  ...
  const save = (info) => {
    ...
    let url = "/cfg.php?controller=exceptDev&action=add_unmanage";
    if (op == "mod") {
      url = "/cfg.php?controller=exceptDev&action=modify_unmanage";
    }
    post(url, data)
      .then((res) => {
        if (!res.success) {
