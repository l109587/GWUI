...
export default () => {
  ...
  const tableKey = "esvrwht"; //table 定义的key
  ...
  const columnvalue = "esvrwhtcolumnvalue"; //设置默认显示的 key 变动 set.strot 存储key
  const apishowurl = "/cfg.php?controller=exceptDev&action=get_servers"; //接口路径
  ...
  const tableTopSearch = () => {
    return (
      <Search
        placeholder={language("ecpmngt.serverwht.tablesearch")}
        style={{ width: 200 }}
        onSearch={(queryVal) => {
  ...
  const save = (info) => {
    ...
    let url = "/cfg.php?controller=exceptDev&action=add_serverwhite";
    if (op == "mod") {
      url = "/cfg.php?controller=exceptDev&action=modify_serverwhite";
    }
    post(url, data)
      .then((res) => {
        if (!res.success) {
  ...
  const delList = (selectedRowKeys) => {
    ...
    post("/cfg.php?controller=exceptDev&action=del_serverwhite", { ids: ids })
      .then((res) => {
        if (!res.success) {
          message.error(res.msg);
          return false;
  ...
  const uploadText = <Button className="llbuttonbox">预览</Button>;
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <EditTable
          label={language("ecpmngt.iporipaddr")}
          name={"addrlistinfo"}
      ...
      <DrawerForm
        ...
        autoFocusFirstInput
        drawerProps={{
          className: "uploadsvrwhtdrawfrom",
          destroyOnClose: true,
          maskClosable: false,
          placement: "right",
          getContainer: false,
