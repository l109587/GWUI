const testMock = (getParam, postParam, res) => {
  let json = "";
  ...
  let list;
  list = require("./json/temporary/montasktable.json");
  if (getParam.action == "showMonitorTask") {
    json = list;
  } else if (getParam.action == "setMonitorTask") {
    if (postParam.op == "add") {
      list.data.push({
        id: Date.now(),
        status: postParam.status,
        name: postParam.name,
        iprange: postParam.name,
        completed: Date.now(),
        port: 1,
        schedule: 3,
        children: [],
      ...
      list.data.map((item, index) => {
        if (item.id == postParam.id) {
          let values = postParam;
          (values.completed = item.completed),
          (values.port = item.port),
          (values.schedule = item.schedule),
          (values.children = item.children),
          delete values.token;
          delete values.op;
          list.data.splice(index, 1, values);
          list.total = list.data.length;
        }
      });
    }
  ...
  } else if (getParam.action == "delMonitorTask") {
    list.data = list.data.filter((item) => item.id != postParam.id);
    list.total = list.data.length;
    json = {
      success: true,
      msg: "操作成功",
