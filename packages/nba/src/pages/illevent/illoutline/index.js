...
import { Space, message, Tag, Input, DatePicker, Select, Tooltip, Spin } from 'antd';
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        if(text == 1) {
          ...
          color = 'purple'
          text = language('illevent.illoutline.findType.eight')
        }
        return (
          <Space>
        ...
        toolBarRender={() => [
          <Tooltip title={language('project.export')} placement='top'>
            <DownloadOutlined style={{ fontSize: '15px' }} onClick={() => {
              ...
              data.user = userValue ? userValue : store.get('userVal');
              download(
                '/cfg.php?controller=outlineEvent&action=export',
                data,
                setDownLoading
              )
            }} />
