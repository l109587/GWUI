...
const Report = () => {
  ...
  const editReport = (values) => {
    setModalStatus(true)
    setRecord(values)
    if(values.type == 'device' || values.type == 'object'){
      setSizeShow(false);
    }else {
      setSizeShow(true);
    }
  ...
  return (
    <>
      ...
      >
        ...
        />
        {sizeShow ? 
        <ProFormDigit label="上报条数"  tooltip='条/次' name="size" min={1} width={100} />
        :<></>}
        <ProFormDigit label="上报间隔" tooltip='分钟'  name="interval" min={1} width={100} />
      </ModalForm>
    </>
  )
}
