...
import { Row, Space, Form, Input, Button, Col, Tag, Select, DatePicker, Modal } from 'antd'
import { layout, rTime, timestampToTime, modalFormLayout } from '@/utils/helper'
...
const Off = () => {
  ...
  let searchVal = { queryVal: queryVal, queryType: 'fuzzy', startTime: olddate, endTime: newdate, class: 1, category: 1 };//顶部搜索框值 传入接口
  const [incID, setIncID] = useState(0);//递增的id 删除/添加的时候增加触发刷新
  const tableKey = 'off';
  const columnvalue = 'offcolumnvalue';
  ...
  return (<>
    <ProtableModule tableKey={tableKey} concealColumns={concealColumns} rowkey={rowkey} rowSelection={false} clientHeight={clientHeight} columns={columns} apishowurl={apishowurl} searchText={tableTopSearch()} searchVal={searchVal} incID={incID} columnvalue={columnvalue} />
  </>)
}
