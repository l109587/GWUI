...
const Multilayer = () => {
  ...
  let searchVal = { queryVal: queryVal, queryType: 'fuzzy', startTime: olddate, endTime: newdate, class: 1, category: 3 };//顶部搜索框值 传入接口
  const [incID, setIncID] = useState(0);//递增的id 删除/添加的时候增加触发刷新
  const tableKey = 'multilayer'
  const rowkey = (record => record.id);
  const columnvalue = 'multiaycolumnvalue';
  ...
  return (<>
    <ProtableModule tableKey={tableKey} concealColumns={concealColumns} rowkey={rowkey} columnvalue={columnvalue} rowSelection={false} clientHeight={clientHeight} columns={columns} apishowurl={apishowurl} searchText={tableTopSearch()} searchVal={searchVal} incID={incID} />
  </>)
}
