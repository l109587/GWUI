...
import { Space, Card, Form, Input, Button, Col, Table, Select, DatePicker, Modal } from 'antd'
...
const Malicious = () => {
  ...
  const apishowurl = '/cfg.php?controller=ntaAlarmLog&action=showMalwareAlarm';
  let searchVal = { queryVal: queryVal, queryType: 'fuzzy', startTime: olddate, endTime: newdate };//顶部搜索框值 传入接口
  const [incID, setIncID] = useState(0);//递增的id 删除/添加的时候增加触发刷新
  const tableKey = 'malicious'
  const rowkey = (record => record.id)
  const columnvalue = 'maliciouscolumnvalue'
  ...
  return (<>
    <ProtableModule columnvalue={columnvalue} tableKey={tableKey} rowkey={rowkey} concealColumns={concealColumns} rowSelection={false} clientHeight={clientHeight} columns={columns} apishowurl={apishowurl} searchText={tableTopSearch()} searchVal={searchVal} incID={incID} />
  </>)
}
