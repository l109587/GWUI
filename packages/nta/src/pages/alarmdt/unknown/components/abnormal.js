...
const Abnormal = () => {
  ...
  let searchVal = { queryVal: queryVal, queryType: 'fuzzy', startTime: olddate, endTime: newdate, alarmType: 3 };//顶部搜索框值 传入接口
  const [incID, setIncID] = useState(0);//递增的id 删除/添加的时候增加触发刷新
  const tableKey = 'abnormalalarm';
  const rowkey = (record => record.id);
  const columnvalue = 'abnormolumnvalue'
  ...
  return (<>
    <ProtableModule tableKey={tableKey} columnvalue={columnvalue} rowkey={rowkey} concealColumns={concealColumns} rowSelection={false} clientHeight={clientHeight} columns={columns} apishowurl={apishowurl} searchText={tableTopSearch()} searchVal={searchVal} incID={incID} downloadButton={downloadButton} />
  </>)
}
