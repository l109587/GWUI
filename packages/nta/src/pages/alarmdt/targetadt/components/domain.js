...
const Domain = () => {
  ...
  const apishowurl = '/cfg.php?controller=ntaAlarmLog&action=showDNSAlarm';
  let searchVal = { queryVal: queryVal, queryType: 'fuzzy', startTime: olddate, endTime: newdate };//顶部搜索框值 传入接口
  const [incID, setIncID] = useState(0);//递增的id 删除/添加的时候增加触发刷新
  const tableKey = 'domain';
  const rowkey = (record => record.id);
  const columnvalue = 'domaincolumnvalue';
  ...
  return (<>
    <ProtableModule columnvalue={columnvalue} tableKey={tableKey} rowkey={rowkey} concealColumns={concealColumns} rowSelection={false} clientHeight={clientHeight} downloadButton={downloadButton} columns={columns} apishowurl={apishowurl} searchText={tableTopSearch()} searchVal={searchVal} incID={incID} />
  </>)
}
